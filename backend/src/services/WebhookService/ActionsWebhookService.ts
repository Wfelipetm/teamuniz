    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (nodeSelected.type === "openai") {
        ...
        let openAiSettings = {
          ...
          model: model || "gpt-4o-mini" // Modelo configurado ou default
        };
      ...
      if (nodeSelected.type === "ticket") {
        ...
        // Garantir que ticket está carregado do DB
        if (!ticket) {
          ticket = await Ticket.findOne({ where: { id: idTicket, whatsappId, companyId } });
        }
        ...
        if (queueId) {
          ...
          // Sem fila definida: apenas tira do flow e deixa pending sem fila
          await ticket.update({
            status: "pending",
            flowWebhook: false,
            lastFlowId: nodeSelected.id,
      ...
      if (!pressKey && !isContinue) {
        ...
        if (nextNode === 0) {
          ...
          // Recarrega ticket para ver se um nó ticket já mudou o status
          const ticketFinal = await Ticket.findOne({
            where: { id: idTicket, whatsappId, companyId: companyId }
          });
          // Só fecha se o nó ticket NÃO foi acionado (status ainda não é pending/open)
          const finalStatus = ticketFinal?.status === "pending" || ticketFinal?.status === "open"
            ? ticketFinal.status  // respeita o nó ticket
            : "closed";          // sem nó ticket → fecha ticket
          await ticket.update({
            status: finalStatus,
            lastFlowId: nodeSelected.id,
            hashFlowId: null,
