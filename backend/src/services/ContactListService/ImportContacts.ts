...
export async function ImportContacts(
  ...
  const contacts = rows.map(row => {
    ...
    // Ajuste: Sempre converte o número para string antes do replace
    if (
      has(row, "numero") ||
      has(row, "número") ||
      has(row, "Numero") ||
      ...
      number = String(numRaw).replace(/\D/g, ""); // converte para string antes do replace
    } else {
      number = ""; // valor default vazio se não tem nenhuma coluna de número
    }
  ...
  if (contactList) {
    for (let newContact of contactList) {
      try {
        ...
        // Ajuste: sempre converte para string antes do replace
        const number = String(response.jid || "").replace(/\D/g, "");
